...
def collect(conn: Any, *, days: int = 7) -> dict:
    ...
    return {
        ...
        "guard_activity": queries.guard_activity(conn, days=days),
    }
...
def render(data: dict, *, file=None) -> None:
    ...
    _render_guard_activity(data.get("guard_activity", []), file=file)
...
def _render_guard_activity(rows: list[dict], *, file) -> None:
    ...
    by_day_company: dict[tuple, dict[str, int]] = {}
    for r in rows:
        key = (r["day"], r["company"])
        by_day_company.setdefault(key, {})[r["status"]] = int(r["count"])

    print(
        f"  {'Company':<32} {'Day':<12} {'Succeeded':>10} {'Cancelled':>10} {'Failed':>8}",
        file=file,
    )
    print(f"  {'-'*32} {'-'*12} {'-'*10} {'-'*10} {'-'*8}", file=file)
    for (day, company), counts in sorted(by_day_company.items(), key=lambda kv: kv[0][0], reverse=True):
        succeeded = counts.get("succeeded", 0)
        cancelled = counts.get("cancelled", 0)
        failed = counts.get("failed", 0)
        flag = "  !!" if cancelled > succeeded else ""
        print(
            f"  {company:<32} {str(day):<12} {succeeded:>10,} {cancelled:>10,} {failed:>8,}{flag}",
            file=file,
        )
    print(file=file)
